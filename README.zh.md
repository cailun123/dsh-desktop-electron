...
启动瞬间即现的动画窗口 —— 一只呼吸的鲸鱼，加一行打字机式的字标：

- 单色底面正中的鲸鱼，2.8 秒一个呼吸周期（缩放 1 → 1.05，不透明度 0.7 → 1）。鲸鱼下方，字标 `DEEPSEEK HARNESS` 逐字打出，充当进度读数 —— 节奏恒定、绝不卡顿（刻意设计的假进度）；打完之后一枚 DeepSeek 蓝的方块光标持续闪烁，让载入状态始终"活着"。没有进度条控件、没有状态行、没有转圈加载器、没有光环与渐变。
- 配色跟随系统与**主程序（dsh GUI）主题**（从 `settings.yaml` 读取）：日间纸白黑鲸、夜间石墨黑白鲸。闪烁光标是整页唯一的点缀色，取 harness 品牌蓝。
- GUI 在动画层背后预载，动画结束时正好是主界面 —— dsh 自己的转圈载入界面全程被盖住。
- 启动画面的独立浏览器预览在 [preview/splash-preview.html](preview/splash-preview.html)：直接双击打开即可（无需启动应用、无网络依赖）查看呼吸 Logo 与就绪交接效果。

### 反转式标题栏

对齐 Codex，但层级反转：窗口**没有原生标题栏** —— 外壳在页面顶部画一条真正的标题栏带，是主界面的元素去侵占标题栏，而不是标题栏的元素侵占主页面。

- Windows（Linux 同方案）上标题栏带横贯整个顶边：一条可拖拽的带状区，用 GUI 自己的边框色 token 画出细线，Window Controls Overlay 的窗口控制按钮就放在这条带里。页面主体——对话区顶部条、输入框、右侧面板——都从标题栏带**下方**开始，关闭按钮不再浮于页面内容之上，任何表面都无需为控制按钮挖出安全边距。
- **左侧功能栏覆盖标题栏**：侧栏保持全高列，其品牌行仍是可拖拽的窗口饰边，整个程序读起来是侧栏贯穿标题栏的一块连续表面。
- macOS 上红绿灯内嵌在侧栏左上角（品牌行保留左侧安全边距），对话区顶部条仍是拖拽区。
- 控制按钮的配色跟随 GUI 的实时主题 —— 深色表面白字形、浅色表面黑字形 —— 同步信号取自 GUI 主题模块自己发布的 theme-color，全程无 preload、无 IPC 通道。

### 窗口与渲染进程安全

- 沙箱化渲染进程（`sandbox: true`、`contextIsolation: true`、`nodeIntegration: false`、无 preload）—— GUI 就是一个普通 Web 应用。
- 任何打开新窗口或导航离开服务端 origin 的行为都转交系统浏览器，且限定 `http(s)`；无法解析的目标直接丢弃。
- 服务端始终监听 `127.0.0.1`，端口由操作系统分配（`--port 0`），因此永远不会和已有的 `dsh web` 冲突。
- 启动时带 `--no-open`，`dsh web` 不会自行打开浏览器标签页 —— 外壳窗口是唯一的 GUI，浏览器实例仍可与外壳同时开着。

### 托盘常驻

- 关闭窗口只是隐藏，服务端继续运行。只有**退出**才终止服务端。
- 托盘菜单对齐 Codex：**新话题**、正在运行的**话题**与最近**话题** —— 点一下直达那个会话（数据来自服务端自身的 session-list RPC；侧栏行点击为尽力而为）。已归档的话题会被过滤：feed 还会读取工作区的归档集合（服务端 `workspace/follow` 流的首帧基线），取不到时沿用上一次的集合，WebSocket 抖动绝不会让已归档话题"复活"。
- 菜单与悬停提示跟随 dsh 的 `locale.preference` 设置（中/英，未设置则跟随系统语言）。
- 托盘图标始终与其所在表面反色以增强可读性：深色任务栏/菜单栏用白色鲸鱼，浅色用黑色鲸鱼。Windows 上读取的是任务栏所跟随的**系统**模式（注册表 `SystemUsesLightTheme`），而非 `nativeTheme` 报告的应用模式，因此常见的"深色任务栏 + 浅色应用"设置下图标依然清晰可辨。
...
```sh
npm test                        # 127 个无密钥用例：命令解析与 spawn、就绪行解析、HTTP 轮询、进程树终止、启动动画时间线、会话 feed、标题栏融合

...
对齐 Codex：窗口**没有原生标题栏** —— GUI 自己的顶部条就是标题栏，整个程序读起来是一块连续的表面，而不是"浏览器框住的页面"。

- 侧栏品牌行与对话区顶部条（面包屑、标签页、操作按钮）成为可拖拽的标题栏；其中的每个控件都保持可点击。英雄态（尚未打开会话）时，居中输入框上方的空白条同样可拖拽。
- 系统窗口控制按钮直接**叠画在**网页表面之上：Windows 用 Window Controls Overlay（Linux 同方案），macOS 用内嵌红绿灯。GUI 顶部条自动留出安全边距，任何可交互元素都不会落到控制按钮底下。
...
```sh
npm test                        # 116 个无密钥用例：命令解析与 spawn、就绪行解析、HTTP 轮询、进程树终止、启动动画时间线、会话 feed、标题栏融合
